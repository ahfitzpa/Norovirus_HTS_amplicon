...
for file in *R1_001.fastq.gz ; do 
        suffix=$(echo $file | cut -d "_" -f1) 
        mv $file ${suffix}.${experiment_name}_L001_R1_001.fastq.gz
done

for file in *R2_001.fastq.gz ; do 
        suffix=$(echo $file | cut -d "_" -f1) 
        mv $file ${suffix}.${experiment_name}_L001_R2_001.fastq.gz
done

ls *.fastq.gz | awk -F '.fastq.gz' '{print "mv "$0""}' > rename.sh

ls *.gz > newname.txt

sed -i 's/-//g' newname.txt

paste -d' ' rename.sh newname.txt > test.sh
...
##################################################################################################################################################################################################
cp ../names_merged.txt 4.quality_filtered/names_merged.txt
cd 4.quality_filtered

while IFS=, read orig target; do
    mv "$orig" "$target"
done < names_merged.txt

for f in *LP4.fasta; do 
    t=$(basename $f | cut -d  "_" -f1)
    cat ${t}_*.LP4.fasta > ${t}.LP4.fasta
done
...
#################################################################################################################################################################################################################### 
# classify vsearch datasets with RDP noronet classifier
classifier_RDP="/data/Food/analysis/R6564_NGS/amy_fitzpatrick/RDP/rdp_classifier_2.13/dist/classifier.jar"
db_RDP="/data/Food/analysis/R6564_NGS/amy_fitzpatrick/RDP/noronet/noronet_RDP.fasta.properties"
mkdir classified_RDP
java -Xmx8g -jar $classifier_RDP -t $db_RDP -c 0.8 -o classified_RDP/${experiment_name}.classified.tsv 8.cluster/all.otus.fasta
